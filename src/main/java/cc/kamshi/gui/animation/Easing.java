package cc.kamshi.gui.animation;
...
public enum Easing {
    ...
    EASE_IN_OUT_CUBIC(x -> x < 0.5 ? 4 * x * x * x : (float)((x - 1) * (2 * x - 2) * (2 * x - 2) + 1)),
    EASE_IN_QUART(x -> x * x * x * x),
    EASE_OUT_QUART(x -> {
        float res = x - 1;
        return 1 - res * res * res * res;
    }),
    EASE_IN_OUT_QUART(x -> {
        float res = x - 1;
        return x < 0.5 ? 8 * x * x * x * x : 1 - 8 * res * res * res * res;
    }),
    EASE_IN_QUINT(x -> x * x * x * x * x),
    EASE_OUT_QUINT(x -> {
        float res = x - 1;
        return 1 + res * res * res * res * res;
    }),
    EASE_IN_OUT_QUINT(x -> {
        float res = x - 1;
        return x < 0.5 ? 16 * x * x * x * x * x : 1 + 16 * res * res * res * res * res;
    }),
    EASE_IN_SINE(x -> 1 - (float) Math.cos(x * Math.PI * 0.5D)),
    EASE_OUT_SINE(x -> (float) Math.sin(x * Math.PI * 0.5D)),
    EASE_IN_OUT_SINE(x -> (float) (0.5D * (1 - Math.cos(Math.PI * x)))),
    ...
    EASE_OUT_BACK(x -> 1.0F + 2.70158F * (x - 1.0F) * (x - 1.0F) * (x - 1.0F) + 1.70158F * (x - 1.0F) * (x - 1.0F)),
