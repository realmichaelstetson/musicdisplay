...
void main() {
    ...
    // Calculate outline alpha (only the border)
    float thickness = borderThickness > 0.01 ? borderThickness : 1.0;
    float innerDist = dist + thickness;
    float mainAlpha = smoothstep(-aa * 0.5, aa * 0.5, innerDist) - smoothstep(-aa * 0.5, aa * 0.5, dist);

    // MIX COLORS FOR GRADIENT with Angle support
    ...
    if (shadowBlur > 0.1) {
        ...
        bloomAlpha *= smoothstep(-aa * 0.5, aa * 0.5, innerDist);
    }
